...
HERG_successful <- HERG_observations %>%
  filter(thieves_A + thieves_J > 0,
         stolen != "unknown") %>%
  mutate(stolen = case_when(stolen == "N,S" ~ "N",
                            TRUE ~ stolen)) %>%
  mutate(for_join = "SUCCESSFUL",
         thief_new = thief,
         count_thieves = case_when(stolen == "N" ~ 0,
                                   stolen == "Y" ~ 1)) %>%
  unite(for_join, for_join, thief, sep = "_", remove = FALSE)
  
HERG_unsuccessful <- HERG_observations %>%
  filter(thieves_A + thieves_J > 0,
         stolen != "unknown") %>%
  mutate(stolen = case_when(stolen == "N,S" ~ "N",
                            TRUE ~ stolen)) %>%
  mutate(for_join = "UNSUCCESSFUL",
         thief_color = thief,
         count_thieves = case_when(stolen == "N" ~ thieves_A + thieves_J,
                                   stolen == "Y" ~ thieves_A + thieves_J - 1))

HERG_new <- full_join(HERG_successful, HERG_unsuccessful)


HERG_new %>%
  group_by(date) %>%
  filter(for_join != "SUCCESSFUL_NA") %>%
  ggplot(mapping = aes(date, count_thieves, fill = for_join)) +
  geom_col() +
  scale_y_continuous(breaks = c(2, 4, 6, 8, 10), limits = c(0,10)) +
  scale_fill_manual(values = c("SUCCESSFUL_A" = "cadetblue2",
                               "SUCCESSFUL_J" = "cadetblue4",
                               "UNSUCCESSFUL" = "black"),
